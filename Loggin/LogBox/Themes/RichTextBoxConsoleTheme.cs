...
namespace Serilog.Sinks.RichTextBox.Themes
{
    ...
    {
        private readonly IReadOnlyDictionary<RichTextBoxThemeStyle, BoxThemeAdonisStyle> _adonis_styles;
        private  IReadOnlyDictionary<RichTextBoxThemeStyle, RichTextBoxConsoleThemeStyle> _styles;

        private const string _runStyleReset = "</Run>";

        void UpdateStyles(object? sender, bool e)
        {
            _styles = _adonis_styles.ToDictionary(kv => kv.Key, kv => (RichTextBoxConsoleThemeStyle) kv.Value);
        }
        /// <summary>
        ...
        public RichTextBoxConsoleTheme(IReadOnlyDictionary<RichTextBoxThemeStyle, BoxThemeAdonisStyle> styles)
        {
            if (styles is null)
            {
                throw new ArgumentNullException(nameof(styles));
            }
            _adonis_styles = styles.ToDictionary(kv => kv.Key, kv => kv.Value);
            _styles = _adonis_styles.ToDictionary(kv => kv.Key, kv => (RichTextBoxConsoleThemeStyle)kv.Value);
            ThemeChangeEvent.ThemeChanged += UpdateStyles;
        }

        /// <summary>
        /// A theme in the style of the original <i>Serilog.Sinks.Literate</i>. This is the default when no theme is specified.
        /// </summary>
        public static RichTextBoxConsoleTheme Monitor { get; } = RichTextBoxConsoleThemes.Monitor;
